...
::: {.fragment}
Thus, it is intuitive to think that: 
...
::: incremental
1. Thurstone: equal dispersion **may not be applicable** when researchers assess complex traits or heterogeneous stimuli 
...
2. Zero correlation is **almost certainly invalid** in two scenarios: 
    ...
    $\longrightarrow$ when intricate aspects of stimuli cause judges' biases, ultimately introducing dependencies between stimuli.
    
    $\longrightarrow$ when an additional hierarchical structure is relevant to the stimuli.
:::
...
$\longrightarrow$ formally and graphically representing the assumed (causal) structure of the CJ system. Specifically we develop:
...
::: incremental
::: {style="font-size:80%;"}
- $n_{I}$ represents the number of students 
- $n_{A}$ denotes the number of texts available per students
- $n_{J}$ indicates the total number of judges, and 
- $n_{K}$ represents the number of judgments each judge performs
- ${n_{I} \cdot n_{A} \choose 2} = \frac{ (n_{I} \cdot n_{A})! }{ 2! (n_{I} \cdot n_{A} - 2)! }$ represents the total number of comparisons
- $n = {n_{I} \cdot n_{A} \choose 2} \cdot n_{J} \cdot n_{K}$ denotes the total number of observation in the data 
:::
:::
...
::: {.fragment}
![](/images/CJ_TM_09.png){width=100%}
:::

Conceptual-population model, final vectorized form. *Left panel* illustrates the SCM. *Right panel * depicts the associated DAG.
:::
...
::: incremental
::: {style="font-size:80%;"}
a. sample of all repeated judgments 
b. sample of judges
c. sample of texts
...
Sample-comparison model including sample and comparison mechanisms. *Left panel* illustrates the SCM. *Right panel * depicts the associated DAG.
:::
...
Sample-comparison model including sample and comparison mechanisms. *Left panel* illustrates the statistical model. *Right panel* depicts the sample-comparison SCM.
:::
...
## 6.1 Findings {style="font-size:80%;"}
...
## 6.2 Limitations and further research {style="font-size:80%;"}
